...
UniValue masternode(const UniValue& params, bool fHelp)
{
        ...
        (strCommand != "start" && strCommand != "start-alias" && strCommand != "start-all" && strCommand != "start-missing" &&
         ...
         strCommand != "connect" && strCommand != "outputs" && strCommand != "status"))
            throw std::runtime_error(
                ...
                "  start        - Start local Hot masternode configured in vivo.conf\n"
    ...
    if (strCommand == "outputs") {
        ...
        BOOST_FOREACH(COutput& out, vPossibleCoins) {	        
            obj.push_back(Pair("Warning", strprintf("This output will be deprecated after block %d:", BLOCKS_AFTER_5000_COLLATERAL_CHANGE)));	
            obj.push_back(Pair(out.tx->GetHash().ToString(), strprintf("%d", out.i)));
		}
...
UniValue masternodelist(const UniValue& params, bool fHelp)
{
                ...
                strMode != "protocol" && strMode != "payee" && strMode != "rank" && strMode != "status"))
    {
        throw std::runtime_error(
                ...
                "  payee          - Print Vivo address associated with a masternode (can be additionally filtered,\n"
                "                   partial match)\n"
